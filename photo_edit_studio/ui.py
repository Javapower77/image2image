...
def _collect_images(
    source: Image.Image | None, references: list[Image.Image] | None
) -> list[Image.Image]:
    images = [source] if source is not None else []
    images.extend(references or [])
    return images
...
def _run(
    ...
    request = GenerationRequest(
        ...
        images=_collect_images(source, references),
        mask=mask,
        width=int(width),
        height=int(height),
        steps=int(steps),
...
def build_app() -> gr.Blocks:
    ...
    with gr.Blocks(title="Local Photo Edit Studio") as app:
        ...
        clear_button.add([source, references, mask, prompt, negative_prompt, output, metadata])
